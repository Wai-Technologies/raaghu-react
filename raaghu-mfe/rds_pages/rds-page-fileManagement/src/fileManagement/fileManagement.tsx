...
import { RdsCompAlertPopup, RdsCompDatatable, RdsCompFileUploader, } from "../../../rds-components";
import RdsCompFileManagementTree, { FileManagementTree } from "../../../../../raaghu-components/src/rds-comp-fileManagement-Tree/rds-comp-fileManagement-Tree"
import {
    RdsAlert,
    RdsBreadcrumb,
    RdsButton,
...
const FileManagement = () => {
    ...
    const fileDescriptor = useAppSelector((state) => state.persistedReducer.fileDescriptors);
    const directoryDescriptor = useAppSelector((state) => state.persistedReducer.directoryDescriptors)
    ...
    useEffect(() => {
        if (directoryDescriptor.getDirectoryDescriptor1?.items) {
            const tempdata = directoryDescriptor.getDirectoryDescriptor1.items.map((item: any) => {
                return {
                    id: item.id,
                    name: item.name,
    ...
    const handlerAction = (rowData: any, actionId: any) => {
      ...
      if (actionId == "download") {
            ...
            dispatch(getFileDescriptorDownloadTokenRequest(dto) as any).then((res: any) => {

                if (res.payload) {

                    dispatch(getFileDescriptorDownloadRequest({ id: dto.id, token: res.payload.token })).then((response: any) => {

                        response.payload.blob().then((res: any) => {

                            const file: any = new File([res], rowData.name, { type: res.type })
    ...
    const UpdateFolderName = () => {
        const dto = { body: { name: name }, id: folderId };
 ...
 const folderMover = (e: any) => {
        ...
        dispatch(postDirectoryDescriptorMoveRequest({ requestBody: files.body }) as any).then((res: any) => {
            if (res.type.includes("rejected")) {
                setAlert({
                    ...Alert,
                    show: true,
            ...
            dispatch(getDirectoryDescriptorSubDirectoriesRequest({ parentId: undefined }) as any);
         });
    ...
    const movefolder = (e: any) => {
        ...
        if (isDirectory) {
            ...
            dispatch(postFileDescriptorMoveRequest({ requestBody: files.body }) as any).then((res: any) => {
                if (res.type.includes("rejected")) {
                    setAlert({
                        ...Alert,
                        show: true,
    ...
    const searchFile = (e: any) => {
        ...
        dispatch(getDirectoryDescriptor1Request({ id: undefined, skipCount: undefined, maxResultCount: undefined, filter: query }));
    }
    ...
    useEffect(() => {
        dispatch(getDirectoryDescriptorSubDirectoriesRequest({ parentId: folderId }) as any);
        
    }, []);

    useEffect(() => {
        if (directoryDescriptor.getDirectoryDescriptorSubDirectories && directoryDescriptor?.getDirectoryDescriptorSubDirectories?.items) {
            const parsedDirectory = JSON.parse(JSON.stringify(directories));
            directoryDescriptor.getDirectoryDescriptorSubDirectories.items.map((el: any) => {
                const tempData = recursiveFunctionAddData(parsedDirectory, el);
    ...
    function handlerPath({ id, name }: any) {
        setFolderId(id);
        dispatch(getDirectoryDescriptor1Request({ id:id, skipCount:skipCount,maxResultCount:recordsPerPage }) as any);
        setbreadCrum(id);
    ...
    const addNewFolder = () => {
        ...
        dispatch(postDirectoryDescriptor1Request({ requestBody: dTo }) as any).then((res: any) => {
            if (res.type.includes("rejected")) {
                setAlert({
                    ...Alert,
                    show: true,
            ...
            dispatch(getDirectoryDescriptorSubDirectoriesRequest({ parentId: undefined }) as any);
            dispatch(getDirectoryDescriptor1Request({ id: folderId, skipCount: skipCount, maxResultCount: recordsPerPage }) as any);
        });
        ...
        if(isDirectory){
            dispatch(deleteDirectoryDescriptorRequest({ id: folderId }) as any).then(
                async  (res: any) => {
                     ...
                     await dispatch(getDirectoryDescriptor1Request({ id: parentFolderId, skipCount: skipCount, maxResultCount: recordsPerPage }) as any);
                  }
              );
        }else{
    ...
    useEffect(() => {
        dispatch(postFileDescriptorPreUploadInfoRequest({ requestBody: uploadFiles }) as any);
    }, [uploadFiles]);
    ...
    const UploadedFile =  () => {
        totalFiles.map((e: any) => {
            const body = {
             ...
             dispatch(postFileDescriptorUploadRequest(body) as any).then(async(res: any) => {
                if (res.type.includes("rejected")) {
                    setAlert({
                        ...Alert,
                        show: true,
            ...
            <div className="container-fluid p-0 m-0">
                <div className="row h-100">
                    ...
                    <div className="col-xxl-7 col-xl-7 col-lg-12 col-12 mb-xxl-0 mb-xl-0 mb-lg-0 mb-4 ps-lg-1 ps-xl-1 ps-xxl-1">
                        <div className="card rounded-0 card-full-stretch border-0 px-4 py-2">
                            {selectedNode === "" && (
                                <>
                                    <div className="card-header bg-transparent px-0 pt-0">
