...
import {getApplications1Request, getApplicationsRequest, putApplicationsRequest, deleteApplicationsRequest, postApplicationsRequest } from "../../../../libs/state-management/applications/applications-slice";
...
const Applications = () => {
  ...
  const permission= useAppSelector((state)=>state.persistedReducer.permissions)
  ...
  useEffect(() => {
    dispatch(getApplications1Request({ skipCount: skipCount, maxResultCount: maxResultCount }) as any);
  ...
  useEffect(() => {
    ...
    if (permission?.getPermissions && permission.getPermissions.groups?.length > 0) {
  ...
  const actions = [
    ...
    { id: "permissions", displayName: t("Permissions"), offId: "application-permissions-off" },
    { id: "chnageHistory", displayName: t("Change History"), offId: "application-changeHistory-off" },
    { id: "delete", displayName: t("AbpUi.Delete"), modalId: "application-delete-off" },
  ];
  const navtabsItems = [
  ...
  const handleActionSelection = (rowData: any, actionId: any) => {
    ...
    if (actionId == "edit") {
      ...
      dispatch(getPermissionsRequest({providerKey:rowData.clientId,providerName:"C"}) as any);
      dispatch(getApplicationsRequest({id:tempApplicationId}) as any);

      if (tempApplicationId == applicationId) {
        handleEditActionData();
      }
      if (permission.getPermissions && (permission.getPermissions?.groups?.length > 0) && (permission.getPermissions.groups == permissionListData)) {
        setPermissionListData(permission.getPermissions.groups);
      }
  ...
  function handleDeleteRecord(e: any) {
    dispatch(deleteApplicationsRequest({id:applicationId}) as any).then((res: any) => {
      dispatch(getApplications1Request({ skipCount: skipCount, maxResultCount: maxResultCount }) as any);
    })
    e.preventDefault();
    setAlertOne(true)
  ...
  function handleApplicationSubmit() {
    dispatch(postApplicationsRequest(basicApplicationData) as any).then((res: any) => {
      dispatch(getApplications1Request({ skipCount: skipCount, maxResultCount: maxResultCount }) as any);
      setActiveNavTabId("0");
    })
  ...
  const handleSaveEditedData = () => {
    const editedScope = scopesListData.filter((item: any) => item.checked).map((item: any) => item.label);
  ...
  const isFormValid = isClientIdValid(basicApplicationData.clientId) && isDisplayNameValid(basicApplicationData.displayName) && basicApplicationData.type !== "" && isValidUrl(basicApplicationData.clientUri) &&
    isValidUrl(basicApplicationData.logoUri) && isValidText(basicApplicationData.redirectUris) && isValidText(basicApplicationData.postLogoutRedirectUris);
                      ...
                      {(activeNavTabIdEdit == "3") && (
                        <>
                          <RdsCompPermissionTree
                            ...
                            selectedPermissions={handleSelectedPermissions}></RdsCompPermissionTree>
                        </>
