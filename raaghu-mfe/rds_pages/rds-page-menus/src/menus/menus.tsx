...
const Menus = () => {
    ...
    const [menusData, setMenus] = useState<any>({
        displayName: "",
        parentId: "",
        pageId: "",
        isActive: true,
    ...
    useEffect(() => {
        if (menuPage.menus) {
            setMenus(menuPage.menus);
            setTreeData(menuPage.menus);
        }
    }, [menuPage.menus]);

    const updatedMenusData = () => {
        if (treeData) {
            const menusData = createTree(
                treeData?.items,
                "parentId",
                "id",
                null,
                        ...
                        <div>
                            <div className="card card-full-stretch rounded-0 border-0">
                                <div className="card-header bg-transparent">
                                    <h5 className="card-title fw-medium">{t("Menu Management Tree")}</h5>
                                </div>
