...
test.describe('RdsSnackbar Component', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(SNACKBAR_STORY_URL);
  ...
  test('should close snackbar after timeout', async ({ page }) => {
    await page.goto(`${STORYBOOK_URL}/iframe.html?id=elements-snackbar--auto-hide&viewMode=story`);
    await page.waitForSelector('.MuiButton-root', { timeout: 10000 });
  ...
  test('should apply different anchor origins', async ({ page }) => {
    await page.goto(`${STORYBOOK_URL}/iframe.html?id=elements-snackbar--error&viewMode=story`);
    await page.waitForSelector('.MuiButton-root', { timeout: 10000 });
