...
export interface NotificationCardProps {
  ...
  showDismiss: boolean;
  onDismiss: (event: React.SyntheticEvent, notification: NotificationItem, notificationIndex: number) => void;
  onAccept?: (event: React.MouseEvent<HTMLElement>, notification: NotificationItem) => void;
}
...
export function NotificationCard({
  ...
  return (
    <Card
      ...
      sx={{
        padding: layout === NotificationLayout.Horizontal && style === NotificationStyle.Image ? 0 : 2,
        marginBottom: 2,
        borderRadius: 2,
        position: "relative",
        display: layout === NotificationLayout.Horizontal && style === NotificationStyle.Image ? "flex" : "block",
        overflow: "hidden",
      }}
    >
      ...
      >
        {showDismiss && (
          <RdsIconButton iconFilled={<Close />} size="small" aria-label="Dismiss notification" onClick={(e) => onDismiss(e, notification, index)} className="rds-comp-notification__dismiss" />
        )}
        ...
        >
          {style === NotificationStyle.Image &&
            (layout === NotificationLayout.Vertical ||
              ...
              <Box className="rds-comp-notification__title-section" sx={{ flexGrow: 1, paddingLeft: 0 }}>
        ...
        <CardContent
          ...
          sx={{
            padding: 0,
            marginTop: 1,
            marginLeft: layout === NotificationLayout.Horizontal && (style === NotificationStyle.Avatar || style === NotificationStyle.Icon) ? 5 : 0,
            "&:last-child": { paddingBottom: 0 },
          }}
        >
          <Typography variant="body2" className="rds-comp-notification__description">
            {description ?? notification.description}
          </Typography>
        </CardContent>
        <Box className="rds-comp-notification__footer" sx={{ display: "flex", justifyContent: "flex-end", marginTop: 2, gap: 1 }}>
          {showButton && (
            <>
              {showSecondaryButton && (
                <RdsButton text="DISMISS" size="small" onClick={handleSecondaryButtonClick} className="rds-comp-notification__secondary-button" />
              )}
              {showPrimaryButton && (
