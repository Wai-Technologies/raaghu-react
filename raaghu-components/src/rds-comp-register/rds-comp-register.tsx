...
const RdsCompRegister = (props: RdsCompRegisterProps) => {
  ...
  const handleRegisterMemberDataChanges = (value: any, key: string) => {
    let errorMessage = "";
    if (key === "password") {
      errorMessage = isNewPassValid(value)
        ? ""
  ...
  function emitMemberSaveData(event: any) {
    event.preventDefault();
    props.onRegisterMemberSaveHandler &&
      props.onRegisterMemberSaveHandler(registerMemberData);
  ...
  const isFormValid =
    ...
    isEmailValid(registerData.emailAddress);
  const isMemberFormValid =
    isUserNameValid(registerMemberData?.userName) &&
    isEmailValid(registerMemberData?.email) &&
    isNameValid(registerMemberData?.name) &&
    isSurnameValid(registerMemberData?.surname) &&
    isPasswordValid(registerMemberData?.password);

  return (
    <>
      {props.register === "default" && (
        <div>
        ...
        <div>
          <form>
            ...
            <RdsButton
              ...
              type="submit"
              dataTestId="register"
              isDisabled={!isMemberFormValid}
              onClick={(e: any) => emitMemberSaveData(e)}
            />
          </form>
        </div>
      )}
