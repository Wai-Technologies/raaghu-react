...
import "./rds-comp-feature-list.css";
...
export interface RdsCompFeatureListProps {
    heading: string
    itemList: string[]
    columns: number,
    fontStyle?: FontStyle;
...
const RdsCompFeatureList = (props: RdsCompFeatureListProps) => {
    ...
    return (
        <>
            <div>
                {props.heading && <div className={` RdsCompFeatureList__Heading ${color}`} role="heading">{props.heading}</div>}
                <div className="row">
                    {columnArray.map(colNumber =>
                        <div key={"col-" + colNumber} className="col" data-testId="column">
                            <ul className="RdsCompFeatureList__Ul">
...
export default RdsCompFeatureList;
