...
import RdsCompRange, { DoubleRangeType, RdsCompRangeProps } from "../src/rds-comp-range/rds-comp-range";


describe("RdsCompRange", () => {
    const defaultProps: RdsCompRangeProps = {
        min: 0,
        max: 100,
        rangeType: "default",
    ...
    it("renders a different type of range when the rangeType prop is set", () => {
        const { container } = render(<RdsCompRange {...defaultProps} rangeType="type1" />);

        const rangeTwo = container.querySelector(".slider_two");
        expect(rangeTwo).toBeInTheDocument();
    });    it("renders the component", () => {
        const { container } = render(<RdsCompRange {...defaultProps} />);
        ...
        const { getAllByRole, container } = render(<RdsCompRange {...defaultProps} />);
