...
import RdsCompBankCardDetail, { RdsCompBankCardDetailProps } from "../src/rds-comp-bank-card-detail/rds-comp-bank-card-detail";
...
const defaultProps: RdsCompBankCardDetailProps = {
    cardDatas: mockCardDatas,
    isSelectable: true,
    isEditable: true,
};

describe("RdsCompBankCardDetail", () => {
    it("renders correctly", () => {
        const { container } = render(<RdsCompBankCardDetail {...defaultProps} />);
        expect(container).toMatchSnapshot();
    });

    it("does not render the edit link when not editable", () => {
        const { queryByText } = render(
            <RdsCompBankCardDetail {...defaultProps} isEditable={false} />
        );
    ...
    it("does not render edit link when not editable (alternate props)", () => {
        const { queryByText } = render(
            <RdsCompBankCardDetail cardDatas={defaultProps.cardDatas} isSelectable={true} isEditable={false} />
        );
        const editLinks = queryByText(/edit/i);
        expect(editLinks).not.toBeInTheDocument();
    });
    ...
    it("renders the radio button when selectable", () => {
        const { getAllByRole } = render(<RdsCompBankCardDetail {...defaultProps} />);
    ...
    it("does not render radio button when not selectable", () => {
        const { queryAllByRole } = render(
            <RdsCompBankCardDetail {...defaultProps} isSelectable={false} />
        );
    ...
    it("renders card details correctly", () => {
        const { getByText } = render(<RdsCompBankCardDetail {...defaultProps} />);
    ...
    it("calls onEdit when edit link is clicked", () => {
        ...
        const { getAllByText } = render(
            <RdsCompBankCardDetail {...defaultProps} onEdit={onEdit} />
        );
