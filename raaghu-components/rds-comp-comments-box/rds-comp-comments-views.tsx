...
import { Divider, IconButton, InputAdornment, TextField, Link } from '@mui/material';
...
import { DropdownMenu } from './rds-comp-comments-box';
import { RdsCommentBoxProps } from './rds-comp-comments-logic-combined';

type CommentsLogic = ReturnType<typeof import('./rds-comp-comments-box').useCommentsBoxLogic>;

interface ViewProps {
  props: RdsCommentBoxProps;
  logic: CommentsLogic;
...
function CommentDropdownMenu({
  ...
  return (
    <div ref={dropdownRef} className="rds-comments-box__comment-dropdown-menu">
      <div className="rds-comments-box__comment-dropdown-item" role="button" tabIndex={0} onClick={() => { onClose(); }} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onClose(); } }}>
        {props.editIcon ? (
...
export function SelectedCommentView({ props, avatarInitials }: ViewProps) {
  return (
...
export function TypingCommentView({ props, logic, avatarInitials }: ViewProps) {
  return (
          ...
          <RdsBox className="rds-comments-box__toolbar-icons">
            <span className="rds-comments-box__emoji-trigger">
              ...
              {logic.emojiPickerOpen && (
                <RdsBox ref={logic.emojiPickerRef} className="rds-comments-box__emoji-dropdown">
                  <RdsEmojiGenerator Type={EmojiGeneratorType.Default} onEmojiSelect={logic.handleEmojiSelect} maxEmojis={60} sx={{ minWidth: 320 }} />
                </RdsBox>
              )}
            </span>
            <span className="rds-comments-box__dropdown-trigger">
              <IconButton aria-label="attach file" className="rds-comments-box__attach-btn" ref={logic.attachBtnRef} onClick={() => logic.setTypingDropdownOpen((open: boolean) => !open)}>
                <AttachmentOutlinedIcon fontSize="inherit" className="rds-comments-box__attach-icon" />
              </IconButton>
              <DropdownMenu visible={logic.typingDropdownOpen} anchorRef={logic.attachBtnRef} onClose={() => logic.setTypingDropdownOpen(false)} onSelect={(label) => logic.setTypingHeader(label)} labels={props.attachmentLabels} />
            </span>
            <IconButton aria-label="mention" className="rds-comments-box__mention-btn" ref={logic.mentionBtnRef} onClick={() => logic.setMentionDropdownOpen((open: boolean) => !open)}>
            ...
            {logic.mentionDropdownOpen && (
              <div className="rds-comments-box__mention-dropdown" ref={logic.mentionDropdownRef}>
                <div className="rds-comments-box__mention-search">
                  <input type="text" placeholder={props.mentionSearchPlaceholder || 'Search'} value={logic.search} onChange={(e) => logic.setSearch(e.target.value)} className="rds-comments-box__mention-search-input" />
                </div>
                ...
                <div className="rds-comments-box__mention-user-list-scroll-hide rds-comments-box__mention-user-list">
                  {logic.filteredUsers.map((user: string) => (
                    <div key={user} className="rds-comments-box__mention-user-item" role="button" tabIndex={0} onClick={() => { logic.setTypingHeader(user); logic.setMentionDropdownOpen(false); }} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); logic.setTypingHeader(user); logic.setMentionDropdownOpen(false); } }}>
                      <span className="rds-comments-box__mention-user-avatar"><AccountCircleIcon className="rds-comments-box__mention-user-icon" /></span>
                      <span className="rds-comments-box__mention-user-name">{user}</span>
                    </div>
                  ))}
                </div>
                <div className="rds-comments-box__mention-invite">
                  <button className="rds-comments-box__mention-invite-btn"><MailOutlineIcon className="rds-comments-box__mention-invite-icon" /> {props.mentionInviteLabel || 'Invite'}</button>
                </div>
              </div>
            )}
...
export function PostedCommentView({ avatarInitials }: ViewProps) {
  return (
...
export function HoverCommentView({ props, logic, avatarInitials }: ViewProps) {
  return (
          ...
          <RdsBox className="rds-comments-box__hover-tools">
            ...
            <span className="rds-comments-box__dropdown-trigger">
              ...
              {logic.hoverDropdownOpen && (
                <CommentDropdownMenu props={props} onClose={() => logic.setHoverDropdownOpen(false)} dropdownRef={logic.hoverDropdownRef} />
              )}
            </span>
          </RdsBox>
        </RdsBox>
...
export function ThreadCommentView({ props, logic, avatarInitials }: ViewProps) {
  return (
        ...
        <RdsBox className="rds-comments-box__thread-icons relative">
          <span className="rds-comments-box__dropdown-trigger">
            ...
            {logic.threadDropdownOpenHeader && (
              <CommentDropdownMenu props={props} onClose={() => logic.setThreadDropdownOpenHeader(false)} dropdownRef={logic.threadDropdownHeaderRef} />
            )}
          </span>
            ...
            <RdsBox className="rds-comments-box__tools relative">
              ...
              <span className="rds-comments-box__dropdown-trigger">
                ...
                {logic.threadDropdownOpenTools && (
                  <CommentDropdownMenu props={props} onClose={() => logic.setThreadDropdownOpenTools(false)} dropdownRef={logic.threadDropdownToolsRef} />
                )}
              </span>
            </RdsBox>
          </RdsBox>
...
SelectedCommentView.displayName = 'SelectedCommentView';
TypingCommentView.displayName = 'TypingCommentView';
PostedCommentView.displayName = 'PostedCommentView';
HoverCommentView.displayName = 'HoverCommentView';
ThreadCommentView.displayName = 'ThreadCommentView';
