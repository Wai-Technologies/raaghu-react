...
export interface RdsAdvanceInputProps {
...
const RdsAdvanceInput = React.forwardRef<HTMLInputElement, RdsAdvanceInputProps>(
  (props, ref) => {
    ...
    const handleCardNumberKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (isNumber.test(key)) {
        if (currentValue.replace(/\s/g, '').length >= 16) {
          e.preventDefault();
          return;
        }
        let newValue = currentValue + key;
        if (newValue.replace(/\s/g, '').length % 4 === 0 && newValue.replace(/\s/g, '').length < 16) {
          newValue += ' ';
        }
        ...
        if (currentValue.endsWith(' ') && newValue.replace(/\s/g, '').length % 4 === 0) {
          newValue = newValue.slice(0, -1);
        }
        inputElement.value = newValue;
        e.preventDefault();
      }
      if (inputElement.value === "" && props.required) {
        setHasError(true);
        setErrorRegardingLengthOrValue("Card Number required.");
      }
    };
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (
        ...
        (currentLength >= maxLength || (e.key === "+" && currentLength > 0))
      ) {
        e.preventDefault();
      }
    };
    ...
    const handlerChange = (e: any) => {
      ...
      if (
        props.inputType === "otp" || props.label === 'Number' &&
      ...
      if (props.inputType === "email") {
        if (inputValue === "") {
          ...
          setErrorRegardingLengthOrValue(
            isValidEmail ? "" : props.validationMsg || "Invalid Email Address."
          );
        }
      }
      
      if (props.inputType === "cardNumber" || props.label === 'Card Number') {
        if (inputValue === "") {
          setIsValid(true);
          setErrorRegardingLengthOrValue("");
        }
    ...
    const inputClasses =
      `form-control ${props.shape ? "shape" : ""} rounded mt-1 form-control-` +
      size +
      " flex-grow-1 " +
      props.customClasses +
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (
                    ...
                    props.label &&
                    ["Phone Number"].includes(props.label)
                  ) {
                    handleKeyDown(e);
                  }
                  if (props.onKeyDown) {
                  ...
                  if (
                    props.inputType === "cardNumber" &&
                    props.label &&
                    ["Card Number"].includes(props.label)
                  ) {
            ...
            <Tooltip text={props.tooltipTitle} place={props.tooltipPlacement}>
              <div className={getClassNames()}>
                <input
                  ...
                  data-testid={props.dataTestId}
                ></input>
                {props.labelPosition === "floating" && props.label && (
                  <label
                    htmlFor={props.id}
...
export default RdsAdvanceInput;
